...
//
//This is the third fully working version of this game, with the added game mechanic of choosing a difficulty to play.
...
void playPong() {
  ...
  bool pong = true;
  int pause = 350;  //Just for default
  ...
  //Initialising the variables for choosing the difficulty
  ...
  while (difficulty > 3) {
    difficulty = 0;
    while (joystickButtonsPressed != true) {
      int originalDifficulty = difficulty;
      readJoysticks();
      ...
      for (int i = 0; i < 5; i++) {
        switch (difficulty) {
          case 0:
            matrix.setColumn(display01, 1 + i, colEasy[i]);
            difficulty = 0;
            break;
          case 1:
            matrix.setColumn(display01, 1 + i, colMedium[i]);
            difficulty = 1;
            break;
          case 2:
            matrix.setColumn(display01, 1 + i, colHard[i]);
            difficulty = 2;
            break;
          case 3:
            matrix.setColumn(display01, 1 + i, colSuperHard[i]);
            difficulty = 3;
            break;
        }
      }
  ...
  spawnBall(); //Spawning and printing the ball for the first time
  matrix.setLed(display01, ballPositionY, ballPositionX, true);
  printBat(0, logicLeftJoystick(joystickLeftY));    //Printing the bats for the first time
  ...
  while (pong == true) {
    ...
    //Setting the length of the pause according to the choosen difficulty
    if ((roundNumber % 6) == 0) {
      if (difficulty == 0) {
        if (pause > 200) {
          pause -= 5; 
...
void moveBall(int difficulty) {
  ...
  //Ball X movement
    ...
    if (difficulty == 2 || difficulty == 3) {
      ...
      if ((((calculateLeftBatPosition == ballPositionY) || (calculateLeftBatPosition + 1 == ballPositionY) || (calculateLeftBatPosition + 2 == ballPositionY)) && (ballPositionX == 1)) || (((calculateRightBatPosition == ballPositionY) || (calculateRightBatPosition + 1 == ballPositionY) || (calculateRightBatPosition + 2 == ballPositionY)) && (ballPositionX == 6))) {
